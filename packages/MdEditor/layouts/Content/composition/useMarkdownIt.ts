...
import { SegmentsResultItem } from 'markdown-it-vue-component';
import {
  computed,
  ComputedRef,
  inject,
...
const useMarkdownIt = (props: ContentPreviewProps, previewOnly: boolean) => {
  ...
  const markHtml = () => {
    ...
    segments.value = getSegmentsComponent(html.value);
  };

  const getSegmentsComponent = (html: string) => {
    const segments = (md as any).getSegments?.(html) ?? [
      { type: 'html', id: 'first', content: html }
    ];
    return segments;
  };
